...
from resumeai_microservice.cache import get_cached_response, set_cached_response, hash_inputs
...
from resumeai_microservice.api.candidate import router as candidate_router
from resumeai_microservice.api.recruiter import router as recruiter_router
...
async def analyze(resume: UploadFile = File(...), job_description: str = Form(...), plan: str = Form("free")):
    ...
    if cached:
        if plan == "free":
            # Example: restrict some details for free users
...
async def optimize(resume: UploadFile = File(...), job_description: str = Form(...), plan: str = Form("free")):
    ...
    set_cached_response("optimize", cache_key, result)
    if plan == "free":
...
async def customize(resume: UploadFile = File(...), job_description: str = Form(...), plan: str = Form("free")):
    ...
    set_cached_response("customize", cache_key, result)
    if plan == "free":
...
async def benchmark(resume: UploadFile = File(...), job_description: str = Form(...), plan: str = Form("free")):
    ...
    set_cached_response("benchmark", cache_key, result)
    if plan == "free":
...
async def ats_scan_endpoint(
    ...
    set_cached_response("ats_scan", cache_key, result)
    if plan == "free":
...
async def generate_cover_letter(job_title: str = Form(...), company: str = Form(...), job_description: str = Form(...), plan: str = Form("free")):
    ...
    set_cached_response("generate_cover_letter", cache_key, result)
    if plan == "free":
...
async def optimize_resume_for_job_endpoint(resume: UploadFile = File(...), job_description: str = Form(...), plan: str = Form("free")):
    ...
    set_cached_response("optimize_resume", cache_key, result)
    if plan == "free":
        return JSONResponse(content={"optimized": result.get("optimized", "")})
    return JSONResponse(content=result)
...
def root():
    ...
    return {"message": "ResumeAI FastAPI microservice is running. See /docs for API documentation."}
